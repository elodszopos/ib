...
describe("IBApi Historical data Tests", () => {
    ...
    test("Option market data", (done) => {
        ...
        let counter = 0;
        ib.once(__1.EventName.connected, () => {
            const contract = new __1.Option("AAPL", "20260717", 300, __1.OptionType.Call);
            ib.reqHistoricalData(refId, contract, "20260622-14:00:00", "30 S", __1.BarSizeSetting.SECONDS_FIFTEEN, __1.WhatToShow.BID_ASK, 0, 2, false);
        }).on(__1.EventName.historicalData, (reqId, time, open, high, low, close, volume, count, WAP) => {
            // console.debug(
            //   "Option market data",
            //   counter,
    ...
    it("Test request tick history", (done) => {
        ...
        ib.on(__1.EventName.connected, () => {
            ib.reqHistoricalTicks(refId, contracts_1.sample_etf, "20240508-17:00:00", "", 10, __1.WhatToShow.TRADES, 0, true);
        }).on(__1.EventName.historicalTicksLast, (reqId, ticks, _done) => {
