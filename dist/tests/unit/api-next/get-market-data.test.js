...
describe("RxJS Wrapper: getMarketData()", () => {
    ...
    test("tickString numeric events", (done) => {
        ...
        const subscription = apiNext
            ...
            next: (data) => {
                if (received == 0) {
                    expect(data.added).toBeDefined();
                    expect(data.changed).toBeUndefined();
                    expect(data.added.get(tickType_1.default.LAST_TIMESTAMP)?.value).toEqual(Number(testValue0));
                }
                else {
                    expect(data.added).toBeUndefined();
                    expect(data.changed).toBeDefined();
                    expect(data.changed.get(tickType_1.default.LAST_TIMESTAMP)?.value).toEqual(Number(testValue1));
                }
                expect(data.all.get(tickType_1.default.LAST_TIMESTAMP)?.value).toEqual(Number(received ? testValue1 : testValue0));
                expect(data.all.get(tickType_1.default.LAST_TIMESTAMP)?.stringValue).toEqual(received ? testValue1 : testValue0);
                expect(data.all.get(tickType_1.default.LAST_TIMESTAMP)?.ingressTm).toEqual(expect.any(Number));
                received++;
                if (received == 2) {
        ...
        api.emit(__1.EventName.tickString, 1, tickType_1.default.LAST_TIMESTAMP, testValue0);
        api.emit(__1.EventName.tickString, 1, tickType_1.default.LAST_TIMESTAMP, testValue1);
    });
    test("tickString structured events", (done) => {
        ...
        const subscription = apiNext
            ...
            next: (data) => {
                expect(data.added).toBeDefined();
                expect(data.changed).toBeUndefined();
                expect(data.all.get(tickType_1.default.RT_VOLUME)?.value).toBeUndefined();
                expect(data.all.get(tickType_1.default.RT_VOLUME)?.stringValue).toEqual(rtVolume);
                expect(data.added.get(tickType_1.default.RT_VOLUME)?.stringValue).toEqual(rtVolume);
                subscription.unsubscribe();
                done();
            },
            error: (error) => {
        ...
        api.emit(__1.EventName.tickString, 1, tickType_1.default.RT_VOLUME, rtVolume);
    });
    test("tickOptionComputationHandler events", (done) => {
        ...
        api.emit(__1.EventName.tickOptionComputation, 1, api_next_1.IBApiTickType.BID_OPTION, tickAttrib, impliedVolatility, delta, optPrice, pvDividend, gamma, vega, theta, undPrice);
    });
