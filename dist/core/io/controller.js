...
const command_buffer_1 = require("../../common/command-buffer");
...
class Controller {
    ...
    constructor(ib, options) {
        ...
        /** The command buffer. */
        this.commands = new command_buffer_1.CommandBuffer(Controller.execute, this);
        ...
        this.rateLimiter = (0, rate_limit_1.rateLimit)(rate / 10, 1000 / 10, (tokens) => {
    ...
    emitError(errMsg, code, reqId, advancedOrderReject) {
        this.emitEvent(event_name_1.EventName.error, new Error(errMsg), code, reqId ?? errorCode_1.ErrorCode.NO_VALID_ID, advancedOrderReject);
    }
    /**
     * Execute a command.
     *
    ...
    executeConnect(clientId) {
        if (!this.socket.connected) {
            ...
            this.emitInfo("Cannot connect if already connected.", errorCode_1.ErrorCode.ALREADY_CONNECTED);
        }
    }
    /**
    ...
    executeSend(tokens) {
        if (this.socket.connected) {
            ...
            this.emitError("Cannot send data when disconnected.", errorCode_1.ErrorCode.NOT_CONNECTED, errorCode_1.ErrorCode.NO_VALID_ID);
        }
    }
}
