...
class ConsoleLogger {
    ...
    warn(tag, args) {
        if (this._logLevel >= __1.LogLevel.WARN) {
            ...
            console.warn(ansi_colors_1.bold.yellow(`[${new Date().toLocaleTimeString()}] [WARN] [${tag}]: `), newArgs);
        }
    }
    /** Log an error. */
    error(tag, args) {
        if (this._logLevel >= __1.LogLevel.ERROR) {
            let newArgs = args;
            if (Array.isArray(args))
                newArgs = args.map((x) => ansi_colors_1.bold.red(util.inspect(x, { showHidden: false, depth: null })));
            console.error(ansi_colors_1.bold.red(`[${new Date().toLocaleTimeString()}] [ERROR] [${tag}]:`), newArgs);
        }
    }
}
