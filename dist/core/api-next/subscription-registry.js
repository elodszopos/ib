...
class IBApiNextSubscriptionRegistry {
    ...
    register(requestFunction, cancelFunction, eventHandler, instanceId) {
        ...
        eventHandler?.forEach((handler) => {
            ...
            let entry = eventEntries.find((eventEntry) => eventEntry.callback === callback);
            if (!entry) {
        ...
        if (!subscription) {
            ...
            entries.forEach((entry) => {
                ...
                entry.subscriptions.set(subscription.reqId, // eslint-disable-line @typescript-eslint/no-non-null-assertion
                subscription);
            });
        }
